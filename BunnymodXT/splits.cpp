...
namespace Splits
{
	...
	void Split::touch()
	{
		...
		if (!found_reached)
		{
			...
			{
				HwDLL::GetInstance().ORIG_Cbuf_InsertText("bxt_timer_reset;bxt_timer_start\n");
				restarting = true;
			}
		...
		if (!restarting && !CustomHud::GetCountingTime())
		{
			// Timer not running and this is not the split that is gonna start it either,
			// so avoid altering state or doing anything...
			return;
		}
		...
		// LiveSplit should have a split with the same name
	...
	const std::string GetSummary(Split& split)
	{
		...
		if (speed)
			oss << dataSeparator << std::hypot(speed.x, speed.y) << " u/s";
	...
	void Reset()
	{
		...
		{
			if (restarting && first && CVars::bxt_splits_start_timer_on_first_split.GetBool())
			{
				// We don't want to clear stuff on the first split, since it has just been used
				// to start the timer and the reached flag at least has to remain set for the rest to work
				// Since use the bxt_timer_reset command and we don't call that command's handler directly,
				// it takes some time and the reached flag is set on touch() before actually resetting the time
				first = false;
				restarting = false;
				continue;
			}
