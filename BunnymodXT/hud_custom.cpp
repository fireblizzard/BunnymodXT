...
namespace CustomHud
{
	...
	void DrawSplit(float flTime)
	{
		...
		// Get the last reached split, so that we can draw its time, unless it was too long ago
		const auto rit = std::find_if(Splits::splits.rbegin(), Splits::splits.rend(), [](Splits::Split& spl) { return spl.get_reached(); });
		if (rit == std::rend(Splits::splits))
		{
			// No split trigger has been reached yet, so there's no time to be drawn
			return;
		}
		const auto split = Splits::splits[std::distance(Splits::splits.begin(), rit.base()) - 1];
		...
		if (CVars::bxt_hud_split_speed.GetBool())
		{
			...
			if (split.get_track_horizontal())
			{
				if (split.get_track_vertical())
					...
					speed = length(vecSpeed.x, vecSpeed.y);
			}
